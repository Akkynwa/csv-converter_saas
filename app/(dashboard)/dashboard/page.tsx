...
import { ReverseConvertButton } from '@/components/ReverseConvertButton';

// 1. History List Component
async function HistoryList({ teamId, isPro }: { teamId: number; isPro: boolean }) {
...
export default async function DashboardPage() {
  ...
  return (
    <section className="relative p-4 lg:p-12 w-full">
      {/* Fixed background pattern */}
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px] opacity-40 pointer-events-none" />
      ...
      <div className="flex flex-col mb-12">
        ...
        <h1 className="text-4xl lg:text-6xl font-black text-gray-900 tracking-tighter leading-none mb-4 uppercase">
      ...
      <DashboardAnimations>
        ...
        <Suspense fallback={<div className="mt-16 h-24 bg-gray-100 animate-pulse rounded-[2rem]" />}>
          <HistoryList teamId={team.id} isPro={isPro} />
        </Suspense>
      </DashboardAnimations>
    </section>
  );
